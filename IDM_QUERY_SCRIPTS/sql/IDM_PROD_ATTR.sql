...
FROM MCMN.MDC_ATTRIBUTE_PRODUCT AS map 
LEFT OUTER JOIN MCMN.ATTR_DEFN_T AS adef ON map.KEYF_ATTRIBUTE=adef.ATTR_ID
LEFT OUTER JOIN MCMN.MDC_ATTRIBUTES_OLD AS aold ON map.keyf_attribute=aold.KEYP_ATTRIBUTE
...
FROM MCMN.MDC_ATTRIBUTE_UPC_OLD AS uatr 
INNER JOIN MCMN.MDC_UPC_OLD AS upc ON uatr.KEYF_UPC=upc.KEYP_UPC
LEFT OUTER JOIN MCMN.ATTR_DEFN_T AS adef ON uatr.KEYF_ATTRIBUTE=adef.ATTR_ID
LEFT OUTER JOIN MCMN.MDC_ATTRIBUTES_OLD AS aold ON uatr.keyf_attribute=aold.KEYP_ATTRIBUTE
WHERE uatr.KEYF_ATTRIBUTE NOT IN 
(SELECT v FROM (VALUES (1), (2), (3), (4), (119), (820), (298)) AS vals(v))
UNION ALL
SELECT upc.KEYF_PRODUCTS AS pid, 'NRF_COLOR' AS aname, CAST(upc.NRF_COLOR AS VARCHAR) AS aval FROM MCMN.MDC_UPC_OLD AS upc 
UNION ALL
SELECT upc.KEYF_PRODUCTS AS pid, 'NRF_SIZE' AS aname, CAST(upc.NRF_SIZE AS VARCHAR) AS aval FROM MCMN.MDC_UPC_OLD AS upc 
ORDER BY pid
WITH UR;
