WITH
                           ...
                           SELECT *, ROW_NUMBER () OVER (PARTITION BY ucolor ORDER BY cnt DESC) AS rn FROM (
                                  ...
                                  SELECT count(uid) AS cnt, ncolor, ucolor FROM (
                                         ...
                                         SELECT uid, ncolor, ucolor FROM (
                                                ...
                                                FROM mcmn.item_t upc inner join mcmn.ITEM_VALID_T iv on iv.UPC_ID = upc.UPC_ID inner join mcmn.WEB_PROD_VALID_T wv on upc.WEB_PROD_ID = wv.WEB_PROD_ID
                  ...
                  (
                    SELECT DISTINCT nrf2, nrf_desc FROM (
                           SELECT nrf2, nrf_desc, ROW_NUMBER () OVER (PARTITION BY nrf2 ORDER BY cnt DESC) as rn FROM (
                                  ...
                                  FROM mcmn.item_t upc inner join mcmn.ITEM_VALID_T iv on iv.UPC_ID = upc.UPC_ID inner join mcmn.WEB_PROD_VALID_T wv on upc.WEB_PROD_ID = wv.WEB_PROD_ID
                                  GROUP BY upc.nrf_clr_nbr, upc.nrf_clr_desc HAVING upc.nrf_clr_desc IS NOT NULL
                                  WITH UR
                           )
                    ) WHERE rn=1
                  ) AS clrdesc
      ...
      upc AS (SELECT * FROM (SELECT UPC.*, ROW_NUMBER () OVER (PARTITION BY clrway_id, upc_clr_nm) AS rn FROM mcmn.item_t upc 
      inner join mcmn.ITEM_VALID_T iv on iv.UPC_ID = upc.UPC_ID inner join mcmn.WEB_PROD_VALID_T wv on upc.WEB_PROD_ID = wv.WEB_PROD_ID
      WITH UR) WHERE rn=1),
...
FROM mcmn.prd_clrway_t pc JOIN mcmn.prod_clrway_img_t pci ON pc.clrway_id = pci.clrway_id
join mcmn.WEB_PROD_VALID_T wv on pc.WEB_PROD_ID = wv.WEB_PROD_ID
